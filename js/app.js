/* ==========================================================================
   Roofing Contractor Template (US) — app.js
...
(function () {
  ...
  function initQuickQuote() {
    var form = $("#quote-form");
    if (!form) return;
    function err(name) { return form.querySelector('[data-error-for="' + name + '"]'); }
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      var name = $("#q-name", form), phone = $("#q-phone", form), zip = $("#q-zip", form);
      var ok = true;
      ok = setErr(name, err("name"), name.value.trim() ? "" : "Enter your name.") && ok;
      ok = setErr(phone, err("phone"), digitsOf(phone.value).length >= 10 ? "" : "Enter a valid phone.") && ok;
      ok = setErr(zip, err("zip"), digitsOf(zip.value).length >= 5 ? "" : "Enter your ZIP.") && ok;
      ...
      var body = [
        "New free-inspection request (hero form)",
        "",
        "Service: " + g("service"),
        "Name: " + g("name"),
        "Phone: " + g("phone"),
      ...
      window.location.href = "mailto:" + email + "?subject=" +
        encodeURIComponent("Free roof inspection request — " + g("name")) +
  ...
  function initLeadForm() {
    ...
    function validateStep(idx) {
      if (idx === 0) {
        ...
        return setErr(null, fieldErr("service"), picked ? "" : "Please pick what you need.");
      }
      if (idx === 2) {
        var ok = true;
        var name = $("#f-name", form), phone = $("#f-phone", form), zip = $("#f-zip", form), email = $("#f-email", form);
        ok = setErr(name, fieldErr("name"), name.value.trim() ? "" : "Please enter your name.") && ok;
        ok = setErr(phone, fieldErr("phone"), digitsOf(phone.value).length >= 10 ? "" : "Please enter a valid phone number.") && ok;
        ok = setErr(zip, fieldErr("zip"), digitsOf(zip.value).length >= 5 ? "" : "Please enter your ZIP code.") && ok;
        if (email && email.value.trim()) {
          var emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value.trim());
          ok = setErr(email, fieldErr("email"), emailOk ? "" : "Please enter a valid email.") && ok;
        } else { setErr(email, fieldErr("email"), ""); }
        return ok;
      }
      return true;
    }
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (status) status.textContent = "Opening your email to send…";
      var href = "mailto:" + email + "?subject=" + encodeURIComponent("Free inspection / estimate request — " + g("name"))
        + "&body=" + encodeURIComponent(lines);
      window.location.href = href;
    });

    $all('input[name="service"]', form).forEach(function (r) {
      r.addEventListener("change", function () { setErr(null, fieldErr("service"), ""); });
    });
  ...
  function initEstimator() {
    var card = $(".estimator__card");
    if (!card) return;
    var empty = $(".est-result__empty", card);
    var value = $(".est-result__value", card);
    var rangeEl = $("[data-range]", card);
    var moEl = $("[data-mo]", card);
    if (!rangeEl || !moEl || !value) return;
    var sel = { material: null, size: null };
    function money(n) { return "$" + Math.round(n).toLocaleString("en-US"); }
    function update() {
      if (!sel.material || !sel.size) return;
      var low = parseFloat(sel.material.getAttribute("data-low"));
      var high = parseFloat(sel.material.getAttribute("data-high"));
      var area = parseFloat(sel.size.getAttribute("data-area"));
      if (!(low > 0) || !(high > 0) || !(area > 0)) return;
      var lo = Math.round(low * area / 500) * 500;
      var hi = Math.round(high * area / 500) * 500;
      var mo = Math.max(5, Math.round((lo / 120) / 5) * 5); // ~120-month financing, $5 steps
      rangeEl.textContent = money(lo) + " – " + money(hi);
      moEl.textContent = "or from " + money(mo) + "/mo with financing";
      if (empty) empty.hidden = true;
      value.hidden = false;
    }
    $all(".est-opt", card).forEach(function (btn) {
      btn.addEventListener("click", function () {
        var group = btn.getAttribute("data-group");
        $all('.est-opt[data-group="' + group + '"]', card).forEach(function (b) {
